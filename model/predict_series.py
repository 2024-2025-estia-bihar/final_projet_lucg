...
def create_features(df):
    ...
    # moyenne mobile
    df_features["temp_rolling_mean_24h"] = (
        df_features["temperature_2m"].rolling(window=8).mean()
    )  # Moyenne mobile sur 24h (8 périodes de 3h)
    df_features["temp_rolling_std_24h"] = (
        df_features["temperature_2m"].rolling(window=8).std()
    )  # Écart-type mobile sur 24h
...
def preprocess_data(df):
    ...
    data_3h["timestamp"] = pd.to_datetime(
        data_3h["date"].astype(str) + " " + data_3h["hour_group"].astype(str) + ":00:00"
    )
...
def train_model(X, y, version):
    ...
    model = RandomForestRegressor(
        n_estimators=200, random_state=42, max_depth=20, min_samples_split=2
    )
    model.fit(X, y)
    ...
    try:
        ...
        model_entry = Model(
            name=model_name, version=version, created_at=created_at, path=model_path
        )
...
def predict(path, X_input):
    ...
    for i in range(len(y_pred)):
        ...
        row = {
            ...
            "timestamp": (
                X.index[i]
                if hasattr(X, "index") and len(X.index) == len(y_pred)
                else None
            ),
        ...
        if model_id:
            ...
            for _, row in result_df.iterrows():
                prediction_entry = Prediction(
                    ...
                    relative_humidity=(
                        str(row["relative_humidity"])
                        if "relative_humidity" in row
                        else None
                    ),
                    precipitation=(
                        str(row["precipitation"]) if "precipitation" in row else None
                    ),
                    surface_pressure=(
                        str(row["surface_pressure"])
                        if "surface_pressure" in row
                        else None
                    ),
