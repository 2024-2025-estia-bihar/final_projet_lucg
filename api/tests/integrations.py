...
class TestAPI(unittest.TestCase):
    ...
    def test_root_endpoint(self):
        ...
        self.assertEqual(response.json(), {"message": "Bienvenue sur l'API de prévision de séries temporelles"})
    ...
    def test_fetch_data_endpoint(self, mock_save, mock_fetch):
        ...
        # Vérifier la réponse
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"message": "24 enregistrements ajoutés à la base de données"})
    ...
    def test_train_model_endpoint(self, mock_training, mock_get_engine):
        ...
        mock_results = [MagicMock(timestamp='2023-01-01 00:00:00', 
                                 temperature_2m='20.0',
        ...
        with patch('api.main.sessionmaker', return_value=mock_session_maker):
            ...
            # Vérifier la réponse
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json(), {"message": "Modèle entraîné avec succès", "version": "1.0.0"})
    ...
    def test_predict_endpoint(self, mock_predict, mock_fetch, mock_get_engine):
        ...
        mock_session.query.return_value.filter.return_value.all.return_value = []  # Pas de données existantes
        mock_session_maker = MagicMock(return_value=mock_session)
        mock_get_engine.return_value = MagicMock()
